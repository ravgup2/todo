...
var createNewTaskElement = function(taskString) {       // New Task List Item
  ...
  var upArrow = document.createElement("i");  // UP Arrow
  var downArrow = document.createElement("i");  // Down Arrow
...
var addTask = function() {                            // Add a new task
  ...
  incompleteTasksHolder.insertBefore(listItem, incompleteTasksHolder.childNodes[0]); // Append the task list item to the #incomplete-tasks at the top
...
var taskCompleted = function() {               // Mark a task as complete
  ...
  completedTasksHolder.insertBefore(listItem, completedTasksHolder.childNodes[0]); // Append the task list item to the #completed-tasks at the top
...
var bindTaskEvents = function(taskListItem, checkBoxEventHandler) {   // Select it's children
  ...
  var upArrow = taskListItem.querySelector("i.up");     //
  var downArrow = taskListItem.querySelector("i.down");     //
  editButton.onclick = editTask;                                      // Bind editTask to edit button
  deleteButton.onclick = deleteTask;                                  // Bind deleteTask to delete button
  upArrow.onclick = moveUP;                                  // Bind moveUP to Up i
  downArrow.onclick = moveDown;                                  // Bind moveDown to down i
